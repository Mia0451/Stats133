...
```{r}
# your code
sum(str_detect(dat$package, "^[a-z]+$"))
```
...
# your code
# 1714
sum(str_detect(dat$package, "^[^aeiouAEIOU]+$"))
```
...
# your code
# 48
sum(str_detect(dat$package,".*[0-9].*[.].*|.*[.].*[0-9].*"))
```
...
```{r}
# your code
all(!str_detect(dat$package, "^[0-9]"))
```
...
```{r}
# your code
all(str_detect(dat$package, "^[A-Za-z]"))
```
...
```{r}
# your code
sum(str_detect(dat$title, "(Bayes|bayes|Bayesian|bayesian)"))
```
...
```{r}
# your code
sum(str_detect(dat$title, "[m|M]odel[s|ing|ling]?"))
```
...
```{r}
# your code
sum(str_detect(dat$title, "(Modeling|Modelling)"))
```
...
```{r}
# your code
sum(str_detect(dat$title, "(Models|Modeling|Modelling)"))
```
...
```{r}
# your code
sum(str_detect(dat$title, "[s|S]tatistic[s|al|ally]?"))
```
...
```{r}
# your code
sum(str_detect(dat$title, "Graph[ics|ical|ing]?"))
```
...
```{r}
# your code
filtered_package = tibble(package= dat$package[str_detect(dat$package, "^[a-z]")])
initial_package = mutate(filtered_package, initial = substring(package, 1, 1))
sorted_data = initial_package |>
  group_by(initial) |>
  count(initial)

ggplot(data = sorted_data, aes(x = reorder(initial, desc(n)), y = n)) + 
  geom_bar(stat='identity') +
  labs(title = "Number of packages by initial letter",
       subtitle = "(Packages with lower case initials)",
       x = "initial",
       y = "count") +
  theme_minimal()
```
...
ggplot(data = sorted_data, aes(x = reorder(initial, desc(n)), y = n)) + 
  geom_bar(stat='identity') +
  labs(title = "Number of packages by initial letter",
       subtitle = "(Packages with upper case initials)",
       x = "initial",
       y = "count") +
  theme_minimal()
```
...
```{r}
# your code
filtered_package = tibble(package= dat$package[str_detect(dat$package, "^[a-zA-Z]")])
initial_package = mutate(filtered_package, initial = substring(package, 1, 1))
sorted_data = initial_package |>
  group_by(lower = tolower(initial)) |>
  count(lower)

ggplot(data = sorted_data, aes(x = reorder(lower, desc(n)), y = n)) + 
  geom_bar(stat='identity') +
  labs(title = "Number of packages by initial letter",
       x = "initial",
       y = "count") +
