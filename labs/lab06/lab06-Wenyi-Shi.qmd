...
```{r}
# your code
slice(starwars, seq(from = 10, to = 50, by = 5))
```
...
```{r}
# your code
filter(starwars, homeworld == "Naboo") |> select(name)
```
...
```{r}
# your code
filter(starwars, sex == "female" & species == "Human") |> select(name, homeworld, species)
```
...
```{r}
# your code
filter(starwars, height < 150) |> arrange(height)
```
...
```{r}
# your code
arrange(starwars, desc(height)) |> slice_head(n = 5) |> select(name, homeworld, species)
```
...
```{r}
# your code
arrange(starwars, desc(mass)) |> slice_head(n = 5) |> select(name, homeworld, species)
```
...
```{r}
# your code
filter(starwars, skin_color == "green") |> select(name)
```
...
```{r}
# your code
filter(starwars, sex == "male" & (hair_color == "brown" | hair_color == "black")) |> select(name, height, mass)
```
...
```{r}
# your code
filter(starwars, sex == "female") |> summarise(sd(height, na.rm = TRUE)) 
```
