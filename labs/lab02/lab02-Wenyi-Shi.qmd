...
```{r}
# your code
area[seq(from = 1, to = length(name), by = 2)]
```
...
```{r}
# your code
income[seq(from = 5, to = length(name), by = 5)]
```
...
```{r}
# your code
population[seq(from = 10, to = length(name), by = 10)]
```
...
```{r}
# your code
name[rev(seq(from = 2, to = length(name), by = 2))]
```
...
#name[region == "West"][which.max(population[region == "West"])]
name[which(region == "West" & population == max(population[region == "West"]))]
```

f)  name of states with illiteracy percentage of at most 60%, and
...
```{r}
# your code
# name[region == "Northeast" & income == "median income"]
