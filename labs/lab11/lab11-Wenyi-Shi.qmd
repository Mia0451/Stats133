...
```{r}
# your code
storms_2000_2009 = storms |> filter(year >= 2000 & year <= 2009)
ggplot() +
  geom_sf(data = world_countries) +
  coord_sf(xlim = c(-110, 0), ylim = c(5, 65)) +
  geom_point(data = storms_2000_2009,
             aes(x = long, y = lat, group = name, color = name),
             size = 0.5, alpha = 0.5) +
  geom_path(data = storms_2000_2009,
            aes(x = long, y = lat, group = name, color = name),
            arrow = arrow(length = unit(0.08, "inches"))) + 
  theme(panel.grid.major = element_blank(),
        axis.ticks = element_blank(), # hide tick marks
        axis.text = element_blank(),
        legend.position = "none") + # hide degree values of lat & lon
...
```{r}
# your code
storms_Septmber_2020 = storms |> filter(year == 2020 & month == 09)
ggplot() +
  geom_sf(data = world_countries) +
  coord_sf(xlim = c(-110, 0), ylim = c(5, 65)) +
  geom_point(data = storms_Septmber_2020,
             aes(x = long, y = lat, group = name, color = name, size = wind),
             alpha = 0.5) +
  geom_path(data = storms_Septmber_2020,
            aes(x = long, y = lat, group = name, color = name),
            arrow = arrow(length = unit(0.08, "inches"))) + 
  theme(panel.grid.major = element_blank(),
        axis.ticks = element_blank(), # hide tick marks
        axis.text = element_blank(),
        legend.position = "none") + # hide degree values of lat & lon
  labs(title = "North Atlantic Storms (Sep 2020)",
       subtitle = "Storm direction and point size indicating the wind",
       x = "", 
       y = "")
```
...
```{r}
# your code
storms_Septmber_2020_sally_beta = storms |> filter(year == 2020 & month == 09 & (name == 'Sally' | name == 'Beta'))
map_usa = ne_states(country = "united states of america", returnclass = "sf")

ggplot() +
  geom_sf(data = map_usa) +
  coord_sf(xlim = c(-110, -75), ylim = c(20, 50)) +
  geom_point(data = storms_Septmber_2020_sally_beta,
             aes(x = long, y = lat, group = name, color = name, size = wind),
             alpha = 0.5) +
  geom_path(data = storms_Septmber_2020_sally_beta,
            aes(x = long, y = lat, group = name, color = name),
            arrow = arrow(length = unit(0.08, "inches"))) + 
  theme(panel.grid.major = element_blank(),
        axis.ticks = element_blank(), # hide tick marks
...
leaflet() |>
  addTiles() |> # default OpenStreetMap map tiles
  addMarkers(lng = c(-122.2626814, -122.2582626), lat = c(37.8710231, 37.8754453), popup = c("Dwinelle Hall", "Foothill Dinning"))
```
...
```{r}
# your code
storms2020 = storms |> filter(year == 2020 & month == 09) |>
  select(name:wind)
...
# Your Turn: Slides Background Image {transition="slide-out" background-image="https://petapixel.com/assets/uploads/2022/07/pillars-of-creation-featured.jpg" background-opacity="0.2" background-size="contain"}
...
## Your Turn: Slides Color and Bullets 

Color the background of this slide and add a bulleted list of cool facts
that appear one at a time :)

::: incremental
1)  I am first bullet.

2)  I am second bullet.

3)  You got it, I am the third bullet.

4)  Getting bored with me, wooo! I am the last one :<
:::

<!-- Hint: transition, background-transition, background-color, and :::incremental or . . . may be useful (in visual mode, insert "slide pause" or "div" with incremental as the class) -->
