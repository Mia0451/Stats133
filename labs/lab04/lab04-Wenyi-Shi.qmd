...
**Your answer: same as before, return the value of the last evaluated
statement**

e)  Does `stats` work for all types of vectors (e.g. logical, numeric,
    character)?

**Your answer: only numeric, logical work, character does not work on
...
**Your answer: Error because the function only need one argument**
...
future_value = function(p = 1, r = 0.01, n = 1, k = 1){
  #first check arguments n
  if(n < 0) {
    stop(print("n cannot be negative"))
  }
  #second check arguments k
  if(k <= 0) {
    stop(print("k must be positive"))
  }
  FV = p * ( 1 + r / k) ** (n * k)
